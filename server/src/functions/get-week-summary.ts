...
export async function getWeekSummary() {
  ...
  const result = await db
    .with(goalsCreatesUpToWeek, goalsCompletedInWeek, goalsCompletedByWeekDay)
    .select({
      completed: sql`(SELECT COUNT (*) FROM ${goalsCompletedInWeek})`.mapWith(
        Number
      ),
      total:
        sql`(SELECT SUM (${goalsCreatesUpToWeek.desiredWeeklyFrequency}) FROM ${goalsCreatesUpToWeek})`.mapWith(
          Number
        ),
